...
class Geometry(object):
    def __init__(self, path):
        ...
        self.dirX = pixel_map[:,10]
        self.dirY = pixel_map[:,11]

        self.pixelPitchInM = 0.0095
        self.focalLengthInM = 4.889
        self.pixelPitchDeg = np.rad2deg(self.pixelPitchInM/self.focalLengthInM)

        self.dirX *= self.pixelPitchDeg
        self.dirY *= self.pixelPitchDeg
        self.fovRadius = 2.25
...
class Cluster(object):
    def __init__(self, event, eps=0.1, min_samples=20, degOverNs=0.15):
        normalizedPhotonStream = event.photonStream.copy()
        normalizedPhotonStream[:,0:2] /= (event.geometry.fovRadius*2.0)
        normalizedPhotonStream[:,2] -= event.minArrivalTime
        normalizedPhotonStream[:,2] /= (event.geometry.fovRadius*2.0)/degOverNs

        dbscan = DBSCAN(eps=eps, min_samples=min_samples).fit(normalizedPhotonStream)
        self.coreSamplesMask = np.zeros_like(dbscan.labels_, dtype=bool)
        self.coreSamplesMask[dbscan.core_sample_indices_] = True
